...
def input_validation(item):
    ...
    #with this small part of the code think of json_input=item as a box.
    #the item represents a box that holds two labels in it, source language code and 
    #target language code.  The two labels are then placed into two separate boxes 
    #which would match the variable names as a way of organizing that input data.
...
#but you would want to set the default level to warning in production to only capture important events.